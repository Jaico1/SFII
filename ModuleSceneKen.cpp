...
ModuleSceneKen::ModuleSceneKen(bool start_enabled) : Module(start_enabled)
{
	...
	girl.frames.push_back({ 623, 15, 33, 56 });
	girl.frames.push_back({ 623, 79, 33, 56 });
	girl.frames.push_back({ 623, 143, 33, 56 });
	girl.speed = 0.08f;
...
update_status ModuleSceneKen::Update()
{
	...
	App->renderer->Blit(graphics, 0, 0, &redShip); // red ship


	// TODO 6: Draw the girl. Make sure it follows the ship movement!
	App->renderer->Blit(graphics, 192, 103, &(girl.GetCurrentFrame()));
	
	App->renderer->Blit(graphics, 0, 170, &ground);
App->renderer->Blit(graphics, 560, 8, &(flag.GetCurrentFrame()), 0.0f); // flag animation
	App->player->Update();
